// subscribe state_msg fro topic /state
...
#include "std_msgs/Float64MultiArray.h" //Xiaoyin
...
private:
    ...
    double desired_acc; //added by Xiaoyin. To replace desired_speed
...
public:
    ...
    RacecarSimulator(): im_server("racecar_sim") {
        ...
        // Determine if we should broadcast   ; bool
        ...
        ////创建一个发布消息类型为sensor_msgs::LaserScan的topic（即scan_topic）的对象（即scan_pub）
        //这里定义了发布topic的名称，其他subscribers想要订阅这个topic的信息只需要知道这个topic的名字是scan_topic就可以了
        //参数1是指queue size，可以理解为通道大小，一般设置为1，即传递的信息是1个接着1个，不是多个齐头并进传输到subscribers处的    

        // Make a publisher for odometry messages ##odom publisher
        ...
        // Make a publisher for ground truth pose ########################### WHy you ara ground truth?
        ...
        // Start a subscriber to listen to drive commands. ##No more need to subscribe from /drive
        //drive_sub = n.subscribe(drive_topic, 1, &RacecarSimulator::drive_callback, this);
        
        // Xiaoyin:A subscriber to listern to state_msg -----------------------------------------------------
        state_sub = n.subscribe("/RC01s/data", 1, &RacecarSimulator::state_callback, this);
        //Xiaoyin: just sub steer msg from carsimCmd topic
        steer_sub = n.subscribe("/RC01s/carsimCmd",1, &RacecarSimulator::steer_callback, this);
        ...
        // Start a subscriber to listen to pose messages # What is pos????
        ...
        clear_obs_button.pose.position.x = 4.71;
        clear_obs_button.pose.position.y = 3;
        ...
        clear_obs_marker.scale.x = 0.6;//clear_obs_button.scale*0.45;
        clear_obs_marker.scale.y = 0.4;//clear_obs_button.scale*0.65;
        clear_obs_marker.scale.z = 0.1;//clear_obs_button.scale*0.45;
        ...
        // enlightening the targe parking region 
        
        
        // visualization_msgs::Marker target_spot;
        // target_spot.type = visualization_msgs::Marker::CUBE;
        // target_spot.pose.position.x = 4.6;
        // target_spot.pose.position.y = 3;
        // //target_spot.scale = 1.0;
        // //target_spot.name = "target parking lot";
        // //target_spot.description = " ";
        
        // target_spot.scale.x = 5;
        // target_spot.scale.y = 2.5;
        // target_spot.scale.z = 1;
        // target_spot.color.r = 1.0;
        // target_spot.color.g = 1.0;
        // target_spot.color.b = 0.0;
        // target_spot.color.a = 1.0;
        
        
    }
    // End fo Racesimulator
    void steer_callback(const std_msgs::Float64MultiArray::ConstPtr& msg){
        auto cutler_cmd = msg->data;
        double max_steer = 31 * PI / 180.0;
        state.steer_angle = cutler_cmd[2] * max_steer;
        }
        
    void state_callback(const std_msgs::Float64MultiArray::ConstPtr& msg){
    
        auto cutler_state = msg->data;// already do inside data
        
        state.x = cutler_state[0]; 
        state.y = cutler_state[1];
        //std::cout << "state.x:"<<state.x<<std::endl;
        //std::cout << "state.y:"<<state.y<<std::endl;
        //state.steer_angle = mb_state[2];
        //state.velocity = mb_state[3];
        double vx = cutler_state[2];
        double vy = cutler_state[3];
        state.velocity =  pow(  ( pow( vx,2 )+ pow( vy,2 ) )  , 0.5  );
        state.theta = cutler_state[4] ;//orientation; yaw angle
        state.angular_velocity = cutler_state[5];// yaw rate, angluar velocity
        
        ROS_INFO_STREAM("state.x: " << state.x);
        ROS_INFO_STREAM("state.y: " << state.y);
        ROS_INFO_STREAM("state.theta, yaw:: " << state.theta);
    ...
    void update_pose(const ros::TimerEvent&) {
        ...
        // Update the pose ####################### Now call the STKinematics function
        ros::Time timestamp = ros::Time::now();  // current time
        // double current_seconds = timestamp.toSec();
        
        // accel = desired_acc;
        // state = STKinematics::update(
        //     state, // the first state is initialized above
        //     accel,
        //     steer_angle_vel,
        //     params,
        ...
        pub_pose_transform(timestamp); // this function will also take state.x, state.y, state.theta as input 

        /// Publish the steering angle as a transformation so the wheels move
        pub_steer_ang_transform(timestamp); // used state.steer_angle

        // Make an odom message as well and publish it
        pub_odom(timestamp); //############################ ODOM 
        //Used state,x;xtste.y;state,theta;
    ...
    // COMPUTE ACCEL!    Input is desired_velocity!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //--------------------------------------------------------------------------
    //----------------------------------------------------------------------------
    //-----------------------------------------------------------------
    ...
    //##########fetech data from /drive topic#####################################################3
    void drive_callback(const ackermann_msgs::AckermannDriveStamped & msg) {
        // original code. fetch  speed    & steer_ang
        desired_speed = msg.drive.speed;
        //desired_steer_ang = msg.drive.steering_angle;
        
        // Xiaoyin code. Only fetch acceleration & steering_angle
        desired_acc = msg.drive.acceleration;        
        desired_steer_ang = msg.drive.steering_angle;
    ...
    void clear_obstacles(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback) {
        ...
        // helper function to get kinematic data frm odom msg  , at current time
