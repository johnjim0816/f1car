...
class Drive():
    def __init__(self):
        ...
        process.daemon = True # ?
        process.start()   
    def throttle(self,throttle_value=1):
        status = 7
        ...
        # time.sleep(0.2)
    def steer_left(self,steer_value=1):
        status = 7
        throttle = 1
        steer = steer_value
        self.cmd.data[0]= status #msg.data[0]
        ...
        self.cmd.data[2] = steer #msg.data[2]  
        # time.sleep(0.2)
    def steer_right(self,steer_value=-1):
        status = 7
        throttle = 1
        steer = steer_value
        self.cmd.data[0]= status #msg.data[0]
        ...
        self.cmd.data[2] = steer #msg.data[2]  
        # time.sleep(0.2)
    def drift_left(self,steer_value=1):
        status = 8 # brake
        throttle = 0
        steer = steer_value
        self.cmd.data[0]= status #msg.data[0]
    ...
    def drift_right(self,steer_value=-1):
        status = 8 # brake
        throttle = 0
        steer = steer_value
        self.cmd.data[0]= status #msg.data[0]
